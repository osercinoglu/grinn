...
def getRibeiroOrtizNetwork(pdb,params,resMeanIntEnFile=False,includeCovalents=True,intEnCutoff=1,rmsdEn=5,
	outName='resNetwork'):

	# Get the number of residues
	sys = parsePDB(pdb)
	sys_sel1 = sys.select(params.sel1)
	sys_sel2 = sys.select(params.sel2)
	...
	sys_sels = sys.select(params.sel1 + ' or ' + params.sel2)
	...
	progbar = pyprind.ProgBar(numResidues_sel1*numResidues_sel2)
...
def getProEnNet(inFolder=False,paramsPickleFile=False,resMeanIntEnFile=False,resCorrFile=False,includeCovalents=True,intEnCutoff=1,
	resCorrCutoff=0.4,outPrefix=False):
	
	if inFolder:
		resMeanIntEnFile = os.path.join(inFolder,'energies_intEnMeanTotal.dat')
		pdb = os.path.join(inFolder,'system.pdb')
		if os.path.exists(os.path.join(inFolder,'energies_resCorr.dat')):
			resCorrFile = os.path.join(inFolder,'energies_resCorr.dat')
		
		paramsPickleFile = os.path.join(inFolder,'params.pkl')

	if not resMeanIntEnFile:
		print('You have to provide the mean interaction energy file.')
		raise SystemExit(0)

	if not paramsPickleFile:
		print('You have to provide the full path of the params.pkl file generated by gRINN.')
		raise SystemExit(0)

	f = open(paramsPickleFile,'rb')
	params =pickle.load(f)
	f.close()

	networkRO = getRibeiroOrtizNetwork(pdb=pdb,params=params,resMeanIntEnFile=resMeanIntEnFile,includeCovalents=includeCovalents,
