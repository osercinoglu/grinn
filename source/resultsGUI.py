    ...
    def __init__(self, parent=None):
        ...
        try:
            ...
            QMessageBox.critical(self, 'PyMol can''be initialized',
                                 "{0}\n\nPyMol widget could not be initialized. Molecule viewer will not start, "
                                 "however the rest of the UI will be functional. Upgrading your graphics card drivers "
                                 "may solve this issue.".format(
                                     repr(
                                         instance)))
...
#        self.scrollArea_tabPairWiseEnergiesBarPlot.verticalScrollBar().valueChanged.connect(self.refreshScrollArea)

#    def refreshScrollArea(self):
#        print('callback is working.')
#        self.scrollArea_tabPairWiseEnergiesBarPlot.repaint()
    ...
    def populateGUI(self):
        ...
        trajPath = os.path.join(
            self.viewResultsParams.outputFolder, 'traj.dcd')
        ...
        if os.path.exists(trajPath) and hasattr(self, "ProteinView"):
            buttonReply = QMessageBox.question(
                self, 'Trajectory found',
                "A trajectory exists in your output folder. Would you like to load it as well?\n"
            ...
            if buttonReply == QMessageBox.No:
                ...
                self.horizontalSlider.setMaximum(
                    self.viewResultsParams.numFrames - 1)  # Remember first frame is already the PDB
    ...
    def updateProteinResidueQuadruples(self):
        ...
        self.ProteinView._pymol.cmd.set('label_size', '-2')
        self.ProteinView._pymol.cmd.label(
            'resi ' + selectedPair1[0][4:] + ' and chain ' + selectedPair1[0][0] + ' and name CA',
            '"%s%s%s" % (chain,resn,resi)')
        self.ProteinView._pymol.cmd.label(
            'resi ' + selectedPair1[1][4:] + ' and chain ' + selectedPair1[1][0] + ' and name CA',
            '"%s%s%s" % (chain,resn,resi)')
        self.ProteinView._pymol.cmd.label(
            'resi ' + selectedPair2[0][4:] + ' and chain ' + selectedPair2[0][0] + ' and name CA',
            '"%s%s%s" % (chain,resn,resi)')
        self.ProteinView._pymol.cmd.label(
            'resi ' + selectedPair2[1][4:] + ' and chain ' + selectedPair2[1][0] + ' and name CA',
            '"%s%s%s" % (chain,resn,resi)')
        self.ProteinView._pymolProcess()
