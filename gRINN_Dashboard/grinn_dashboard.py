...
def main():
    ...
    def _sort_residues_by_metric(df: pd.DataFrame, metric: str, residues: list, order: str) -> list:
        ...
        """
        # 'cluster' mode is handled later in the heatmap callback (needs the
        # full residue × frame matrix); short-circuit here as a no-op.
        if order in ('sequence', 'cluster') or df is None or df.empty:
            return residues
    ...
    # Disclaimer prepended to every "Explain biologically" response so the caveat
    # is the first thing the reader sees and travels with the message if copied.
    EXPLAIN_BIO_DISCLAIMER = (
        "> ⚠️ _This biological interpretation is generated by a language model "
        "from a finite-frame MD analysis. Treat it as a hypothesis to validate "
        "against literature, replicate simulations, and experimental data — "
        "not as a biological conclusion._\n\n"
    )
                                            ...
                                            html.Div(style={'display': 'flex', 'alignItems': 'center', 'gap': '6px'}, children=[
                                                ...
                                                dcc.Dropdown(
                                                    ...
                                                    options=[
                                                        ...
                                                        {'label': 'Cluster (similar profiles)', 'value': 'cluster'}
                                                    ],
                                                    value='sequence',
                                                    clearable=False,
                ...
                dbc.CardBody([
                    html.Div(
                        [
                            ...
                            # "Explain biologically" button + persistent caveat caption
                            html.Div([
                                dbc.Button(
                                ...
                                html.Div(
                                    '⚠️ Hypothesis-generation aid — LLM-generated from finite-frame MD data. '
                                    'Verify against literature, replicate simulations, and experiment.',
                                    style={
                                        'fontSize': '10px',
                                        'color': '#856404',
                                        'fontStyle': 'italic',
                            ...
                            ], id='explain-bio-container', style={'display': 'none', 'flex': '0 0 auto'}),
                            html.Div(
    ...
    # Clientside callback: sync the dendrogram subplot's y-axis range with the
    # heatmap subplot's when the user zooms/pans/resets the heatmap in cluster
    # mode (R3.c follow-up). Plotly's `matches='y2'` proved inert in this
    # subplot configuration; we instead listen on relayoutData and write the
    # heatmap's yaxis2 range back to yaxis. One-way (heatmap drives dendrogram).
    app.clientside_callback(
        """
        function(relayoutData, currentFigure) {
            if (!relayoutData || !currentFigure) {
            ...
            // Only react to events that touch the heatmap's y-axis (yaxis2).
            // Skip events that mention only yaxis (likely our own write-back),
            // dragmode toggles, hover changes, etc.
            var hasY2 = keys.some(function (k) {
                return k === 'yaxis2.range[0]' || k === 'yaxis2.range[1]'
                    || k === 'yaxis2.autorange';
            });
            if (!hasY2) {
                return window.dash_clientside.no_update;
            }
            var fig = JSON.parse(JSON.stringify(currentFigure));
            fig.layout = fig.layout || {};
            fig.layout.yaxis = fig.layout.yaxis || {};
            if (relayoutData['yaxis2.autorange'] === true) {
                fig.layout.yaxis.autorange = true;
                delete fig.layout.yaxis.range;
            } else {
                var r0 = relayoutData['yaxis2.range[0]'];
                var r1 = relayoutData['yaxis2.range[1]'];
                if (r0 === undefined || r1 === undefined) {
                    return window.dash_clientside.no_update;
                }
                fig.layout.yaxis.range = [r0, r1];
                fig.layout.yaxis.autorange = false;
            }
            return fig;
        }
        """,
        Output('network_metrics_heatmap', 'figure', allow_duplicate=True),
        Input('network_metrics_heatmap', 'relayoutData'),
        State('network_metrics_heatmap', 'figure'),
        prevent_initial_call=True,
    )
    ...
    @app.callback(
        ...
        Output('explain-bio-container', 'style'),
    ...
    def _on_chat_msg(new_message, messages, sid, selected_model, selected_dfs, token_data,
        ...
        try:
            ...
            explain_btn_style = {'display': 'block', 'flex': '0 0 auto'}
        except Exception as e:
            err_str = str(e).lower()
    ...
    @app.callback(
        ...
        Output('explain-bio-container', 'style', allow_duplicate=True),
    ...
    def _on_explain_stage2(stage_payload, messages, selected_model, charts_store,
        ...
        try:
            ...
            if papers:
                ...
                combined_text += '\n'.join(footer_lines)
            messages.append({'role': 'assistant', 'content': {'type': 'text', 'text': EXPLAIN_BIO_DISCLAIMER + combined_text}})
    ...
    def update_pairwise_graphs(sel1, sel2, selected_frame, second_data):
        ...
        # title='' is explicit so Plotly.react() clears any prior title on the client;
        # without it, a previously-rendered title persists across the empty-state return path.
        def _make_empty_energy_fig():
            fig = go.Figure()
            fig.update_layout(
                title='',
        ...
        total_fig = _make_empty_energy_fig()
        vdw_fig = _make_empty_energy_fig()
        elec_fig = _make_empty_energy_fig()
        bar_fig = go.Figure()
        ...
        for energy_type, config in energy_configs.items():
            ...
            fig.update_layout(
                hovermode='x unified',
                title=dict(text=f"{config['title']} for {first}-{second}"),
                xaxis_title='Frame',
    ...
    def update_network_metrics_visualization(metric, current_frame, n_clicks, selected_residues, energy_type,
        ...
        # R3.c: cluster Y-axis by similar centrality profiles (correlation
        # distance, average linkage). Run before frame_labels / titles so any
        # subsequent code uses the reordered residues.
        cluster_linkage = None  # populated only when sort_order=='cluster' & N>=2
        if sort_order == 'cluster' and len(residues) >= 2:
            mat = np.nan_to_num(heatmap_data, nan=0.0)
            row_var = mat.std(axis=1)
            active_idx = np.where(row_var > 0)[0]
            flat_idx = np.where(row_var == 0)[0]
            if len(active_idx) >= 2:
                d = pdist(mat[active_idx], metric='correlation')
                # Guard against any residual NaNs in the distance vector
                # (correlation distance can produce NaN if rows still degenerate)
                d = np.nan_to_num(d, nan=0.0, posinf=0.0, neginf=0.0)
                cluster_linkage = linkage(d, method='average')
                ordered_active = active_idx[leaves_list(cluster_linkage)]
            else:
                ordered_active = active_idx
            new_order = np.concatenate([ordered_active, flat_idx]).astype(int)
            heatmap_data = heatmap_data[new_order]
            residues = [residues[i] for i in new_order]
        elif sort_order == 'cluster':
            # N < 2: nothing meaningful to cluster; fall through to sequence order
            pass
        ...
        # Build the heatmap trace once; both branches reuse it.
        ...
        if sort_order == 'cluster' and cluster_linkage is not None:
            # Compose dendrogram (left) + heatmap (right) via subplots.
            # Active rows (clustered) live at heatmap rows [0 .. n_active-1];
            # flat rows are appended below and have no dendrogram branches.
            n_active = len(ordered_active)
            dendro = scipy_dendrogram(cluster_linkage, orientation='right', no_plot=True)
            fig = make_subplots(
                rows=1, cols=2,
            ...
            # scipy returns icoord on a 10-spaced lattice [5, 15, 25, …]; map
            # to row indices so leaves line up with heatmap rows. With
            # orientation='right' icoord is the y-axis and dcoord is x.
            for xs, ys in zip(dendro['icoord'], dendro['dcoord']):
                xs_rows = [(v - 5) / 10 for v in xs]
                fig.add_trace(
                    go.Scatter(
                        x=ys, y=xs_rows,
                        mode='lines',
            ...
            # Frame highlight rectangle in the heatmap subplot
            frame_idx = current_frame - frame_min
            fig.add_shape(
                type='rect',
                x0=frame_idx - 0.5, x1=frame_idx + 0.5,
                y0=-0.5, y1=len(residues) - 0.5,
                line=dict(color='red', width=2),
                fillcolor='rgba(0,0,0,0)',
                xref='x2', yref='y2',
            )
            # Dendrogram subplot styling: hide ticks; reverse x-axis so
            # branches grow outward from the heatmap's left edge. Lock the y-range
            # Lock the y-range explicitly so the dendrogram aligns row-for-row
            # with the heatmap on initial render. Zoom/pan sync between the two
            # subplots is handled by a clientside relayoutData callback at the
            # bottom of the dashboard module — Plotly's `matches='y2'` was
            # tried and proved inert in this configuration (subplots +
            # tickmode='array' on the heatmap axis silently disable the link).
            fig.update_xaxes(showticklabels=False, ticks='', showgrid=False,
                             zeroline=False, autorange='reversed', row=1, col=1)
            fig.update_yaxes(showticklabels=False, ticks='', showgrid=False,
                             zeroline=False,
                             range=[-0.5, len(residues) - 0.5],
                             autorange=False,
                             row=1, col=1)
            # Heatmap subplot: tick labels live in the gap between dendrogram and
            # heatmap (seaborn.clustermap layout). Force the clustered residue
            # order via tickmode='array' and lock the range so they stay aligned.
            fig.update_xaxes(title='Frame', tickfont=dict(size=12),
                             side='bottom', showgrid=False, row=1, col=2)
            fig.update_yaxes(title='Residue', tickfont=dict(size=12),
                             showgrid=False,
                             showticklabels=True,
                             tickmode='array',
            ...
            # Vertical line highlighting the current frame
            frame_idx = current_frame - frame_min
            fig.add_shape(
                type='rect',
                x0=frame_idx - 0.5, x1=frame_idx + 0.5,
            ...
            fig.update_layout(
                xaxis=dict(title='Frame', tickfont=dict(size=12),
                           side='bottom', showgrid=False),
                yaxis=dict(title='Residue', tickfont=dict(size=12),
                           showgrid=False, autorange=True),
            )
            title_suffix = '' if sort_order == 'sequence' else f' (sorted {sort_order})'

        # Build the unified title and apply layout common to both branches
        title_text = f'{metric_titles[metric]} Across Frames{title_suffix}'
        fig.update_layout(
            title=dict(
                text=title_text,
                font=dict(size=14, color=soft_palette['primary'], family='Roboto, sans-serif')
